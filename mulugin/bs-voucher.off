...
/* ============================================================
 * 5) АВТООТПРАВКА ПРИ ПОЛНОЙ ОПЛАТЕ
 * ============================================================ *
 * Вместо жёсткой привязки к статусам ('confirmed','approved','paid')
 * проверяем, что бронь ОПЛАЧЕНА (paid >= total) и ваучер ещё не отправлялся.
 */
add_action('mphb_booking_status_changed', function($booking, $new_status, $old_status = null){

	// 1) Получаем ID брони (объект или число)
	$booking_id = 0;
	...
	// 2) Если ваучер уже когда-либо отправлялся (ручной или авто) — не дублируем
	$already_sent = get_post_meta($booking_id, '_bsbt_voucher_sent', true);
	if ($already_sent) {
		return;
	}

	// 3) Получаем суммы total / paid
	$total = 0.0;
	$paid  = 0.0;
	...
	// Фоллбэк: попробуем мета, если по объекту не получилось
	if ($total <= 0) {
	...
	// Если нет общей суммы — нечего проверять
	if ($total <= 0) {
		return;
	}

	// 4) Считаем бронь полностью оплаченной, если paid >= total - 0.01
	if ($paid + 0.01 < $total) {
		// Ещё не полная оплата — выходим
		return;
	}

	// 5) Полная оплата — отправляем ваучер
	...
	// Фактическую отметку "ваучер отправлен" ставит сама bs_bt_send_voucher_email
}, 10, 3);
...
function bs_bt_send_voucher_email($booking_id, $source='auto', $override_email=null) {
	...
	// 🔸 NEW: единый флаг "ваучер уже отправлялся" + источник
