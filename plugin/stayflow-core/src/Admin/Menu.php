...
/**
 * Version: 2.0.0
 * RU: Управление меню. Оживлены вкладки Policies и Content Registry.
 */
final class Menu
{
    public function register(): void
    {
        ...
        // RU: Регистрируем опции для новых страниц
        ...
        <div class="wrap stayflow-admin-wrap">
            ...
            <form method="post" action="options.php">
                ...
                <div class="sf-settings-grid">
                    ...
                    <div class="sf-settings-card">
                        <h3>📄 Owner Automation: PDF E-Mails</h3>
                        <p class="sf-hint">Texte für E-Mails, an die das Owner-PDF angehängt wird.</p>
                        <table class="form-table">
    ...
    public function renderPolicies(): void
    {
        ...
        $flex = $options['free_cancellation'] ?? $def_flex;
        $non_ref = $options['non_refundable'] ?? $def_non_ref;
        ?>
        <div class="wrap stayflow-admin-wrap">
            <h1 class="sf-page-title">🛡️ Cancellation Policies</h1>
            <p style="color: #64748b; margin-bottom: 30px;">Verwalten Sie die Texte für Stornierungsbedingungen. HTML ist erlaubt.</p>
            <?php settings_errors('stayflow_policies_group'); ?>
            <form method="post" action="options.php">
                <?php settings_fields('stayflow_policies_group'); ?>
                <div class="sf-settings-grid">
                    <div class="sf-settings-card">
                        <h3>Flexible Stornierung (Free Cancellation)</h3>
                        <p class="sf-hint">Wird verwendet, wenn der Host flexible Stornierung anbietet.<br><strong>Wichtig:</strong> Verwenden Sie <code>{days}</code> (z.B. "14") und <code>{penalty_days}</code> (z.B. "13") als dynamische Platzhalter!</p>
                        <textarea name="<?php echo $optKey; ?>[free_cancellation]" rows="8" class="large-text" style="font-family: monospace;"><?php echo esc_textarea($flex); ?></textarea>
                    </div>
                    <div class="sf-settings-card">
                        <h3>Nicht erstattbar (Non-Refundable)</h3>
                        <p class="sf-hint">Standard-Text für die nicht erstattbare Rate.</p>
                        <textarea name="<?php echo $optKey; ?>[non_refundable]" rows="10" class="large-text" style="font-family: monospace;"><?php echo esc_textarea($non_ref); ?></textarea>
                    </div>
                </div>
                <?php submit_button('Policies speichern', 'primary', 'submit', true, ['style' => 'background: #082567; border-color: #082567; color: #E0B849; padding: 5px 25px; border-radius: 8px;']); ?>
            </form>
        </div>
        <?php $this->adminStyles(); ?>
        <?php
    ...
    // =========================================================================
    // 4. РЕЕСТР КОНТЕНТА (Content Registry - Vouchers)
    // =========================================================================
    public function renderContentRegistry(): void
    {
        $optKey = 'stayflow_registry_content';
        $options = get_option($optKey, []);
        $def_voucher = "<strong>Check-in:</strong> ab 15:00 Uhr\n<strong>Check-out:</strong> bis 11:00 Uhr\n\nBitte kontaktieren Sie Ihren Gastgeber vorab bezüglich der Schlüsselübergabe.";
        $voucher_text = $options['voucher_instructions'] ?? $def_voucher;
        ?>
        <div class="wrap stayflow-admin-wrap">
            <h1 class="sf-page-title">📝 Content Registry</h1>
            <p style="color: #64748b; margin-bottom: 30px;">Zentrale Verwaltung für dynamische Textbausteine (Voucher, Verträge, etc.).</p>
            <?php settings_errors('stayflow_content_group'); ?>
            <form method="post" action="options.php">
                <?php settings_fields('stayflow_content_group'); ?>
                <div class="sf-settings-grid">
                    <div class="sf-settings-card">
                        <h3>🎟️ Voucher: Check-in / Check-out Instructions</h3>
                        <p class="sf-hint">Dieser Text wird auf den generierten Gast-Vouchern (PDF) angezeigt. HTML erlaubt.</p>
                        <textarea name="<?php echo $optKey; ?>[voucher_instructions]" rows="6" class="large-text" style="font-family: monospace;"><?php echo esc_textarea($voucher_text); ?></textarea>
                    </div>
                </div>
                <?php submit_button('Content speichern', 'primary', 'submit', true, ['style' => 'background: #082567; border-color: #082567; color: #E0B849; padding: 5px 25px; border-radius: 8px;']); ?>
            </form>
        </div>
        <?php $this->adminStyles(); ?>
        <?php
