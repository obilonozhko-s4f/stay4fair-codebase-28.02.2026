...
/**
 * Version: 1.1.4
 * RU: Полная форма редактирования (sf_commune_reg_id, текстовые кнопки удаления, правильный iCal Export).
 * EN: Full edit form (sf_commune_reg_id, text delete buttons, correct iCal Export).
 */
final class ApartmentEditProvider
{
    ...
    public function renderForm(): string
    {
        ...
        // ИСПОЛЬЗУЕМ ПРАВИЛЬНЫЙ КЛЮЧ ДЛЯ MOTO PRESS
        $reg_id      = get_post_meta($apt_id, 'sf_commune_reg_id', true);
        ...
        // --- iCal Links ---
        $ical_import_url = get_post_meta($apt_id, '_sf_ical_import', true);
        $export_url = '';
        $accoms = get_posts([
            'post_type'      => 'mphb_room',
        ...
        if (!empty($accoms)) {
            $room_id = (int)$accoms[0]->ID;
            $export_url = home_url('/?feed=mphb.ics&accommodation_id=' . $room_id);
            $sync_data = get_post_meta($room_id, 'mphb_sync_urls', true);
            if (is_array($sync_data) && isset($sync_data[1]['url'])) {
                $ical_import_url = $sync_data[1]['url'];
            }
        }
            ...
            <form id="sf-edit-form" method="POST" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data">
                ...
                <div style="display: flex; gap: 15px;">
                    <div class="sf-form-group" style="flex:1;">
                        ...
                        <p class="sf-field-hint">Ihr Endpreis inkl. 15% Provision.</p>
                    </div>
                    <div class="sf-form-group" style="flex:1;">
                ...
                <div class="sf-form-group" style="margin-top: 15px;">
                    <label>iCal Kalender-Import</label>
                    <input type="url" name="apt_ical" value="<?php echo esc_attr($ical_import_url); ?>" placeholder="https://...">
                    <p class="sf-field-hint">Fügen Sie hier Ihren iCal-Link (z.B. von Airbnb) ein.</p>
                </div>
